...
CFG_EMPTY_CACHE_BEFORE_BACKWARD="${CFG_EMPTY_CACHE_BEFORE_BACKWARD:-False}"
CFG_LOAD_HF_CHECKPOINT="${CFG_LOAD_HF_CHECKPOINT:-False}"
CFG_HF_CHECKPOINT_FOLDER="${CFG_HF_CHECKPOINT_FOLDER:-local:/MindSpeed-LLM/InternEvo_for10m/llm_ckpts/hf/internlm2-7b}"
CFG_TRAIN_FOLDER="${CFG_TRAIN_FOLDER:-}"
CFG_VALID_FOLDER="${CFG_VALID_FOLDER:-}"
CFG_MICRO_NUM="${CFG_MICRO_NUM:-1}"
CFG_VALID_MICRO_NUM="${CFG_VALID_MICRO_NUM:-4}"
CFG_VALID_EVERY="${CFG_VALID_EVERY:-0}"
CFG_MIN_LENGTH="${CFG_MIN_LENGTH:-50}"
CFG_LR="${CFG_LR:-1e-4}"
CFG_WARMUP_RATIO="${CFG_WARMUP_RATIO:-0.01}"
CFG_ETA_MIN="${CFG_ETA_MIN:-1e-5}"
CFG_USE_PACKED_DATASET="${CFG_USE_PACKED_DATASET:-False}"
CFG_WEIGHT_OVERLAP="${CFG_WEIGHT_OVERLAP:-True}"
...
ECBB_TAG=""
if [ "${CFG_EMPTY_CACHE_BEFORE_BACKWARD}" != "False" ] && [ "${CFG_EMPTY_CACHE_BEFORE_BACKWARD}" != "0" ]; then
  ECBB_TAG="_ecbbTrue"
fi
INIT_TAG="_scratch"
if [ "${CFG_LOAD_HF_CHECKPOINT}" != "False" ] && [ "${CFG_LOAD_HF_CHECKPOINT}" != "0" ]; then
  INIT_TAG="_hfckpt"
fi
DATA_TAG="_randomdata"
if [ -n "${CFG_TRAIN_FOLDER}" ]; then
  DATA_TAG="_realdata"
fi
_LAUNCH_TIMESTAMP=`date +%Y%m%d%H%M%S`
LOG_NAME="${LOG_NAME:-${_LAUNCH_TIMESTAMP}${ALB_TAG}${ECBB_TAG}${INIT_TAG}${DATA_TAG}_seq${SEQ_K}k_steps${CFG_TOTAL_STEPS}_pp${CFG_PP_SIZE}_h${CFG_HEAD_SIZE}_c${CFG_CONTEXT_SIZE}_ws${CFG_WINDOW_SIZE}${FLASH_TAG}}"

export CFG_SEQ_LEN CFG_TOTAL_STEPS CFG_PP_SIZE CFG_HEAD_SIZE CFG_CONTEXT_SIZE CFG_WINDOW_SIZE CFG_USE_FLASH_ATTN CFG_NPU_FIXEDLEN_FLASH_2D CFG_OVERLAP_SYNC_GRAD CFG_OVERLAP_SYNC_PARAM CFG_FIXED_RANDOM_DATASET_SEQLEN CFG_EMPTY_CACHE_AND_DIAG_INTERVAL CFG_EMPTY_CACHE_BEFORE_BACKWARD CFG_LOAD_HF_CHECKPOINT CFG_HF_CHECKPOINT_FOLDER CFG_TRAIN_FOLDER CFG_VALID_FOLDER CFG_MICRO_NUM CFG_VALID_MICRO_NUM CFG_VALID_EVERY CFG_MIN_LENGTH CFG_LR CFG_WARMUP_RATIO CFG_ETA_MIN CFG_USE_PACKED_DATASET CFG_WEIGHT_OVERLAP LOG_NAME
...
echo "CFG_EMPTY_CACHE_BEFORE_BACKWARD=${CFG_EMPTY_CACHE_BEFORE_BACKWARD}"
echo "CFG_LOAD_HF_CHECKPOINT=${CFG_LOAD_HF_CHECKPOINT}"
echo "CFG_HF_CHECKPOINT_FOLDER=${CFG_HF_CHECKPOINT_FOLDER}"
echo "CFG_TRAIN_FOLDER=${CFG_TRAIN_FOLDER:-<synthetic-random>}"
echo "CFG_VALID_FOLDER=${CFG_VALID_FOLDER:-<disabled>}"
echo "CFG_MICRO_NUM=${CFG_MICRO_NUM} CFG_VALID_MICRO_NUM=${CFG_VALID_MICRO_NUM} CFG_VALID_EVERY=${CFG_VALID_EVERY}"
echo "CFG_LR=${CFG_LR} CFG_WARMUP_RATIO=${CFG_WARMUP_RATIO} CFG_ETA_MIN=${CFG_ETA_MIN}"
echo "CFG_USE_PACKED_DATASET=${CFG_USE_PACKED_DATASET}"
echo "CFG_WEIGHT_OVERLAP=${CFG_WEIGHT_OVERLAP}"
